...
class MeetingRecorderApp:
    ...
    def _audio_callback(self, indata, frames, time_info, status) -> None:
        if status:
            self._log(f"Aviso de audio: {status}")
    ...
    def start_recording(self) -> None:
        ...
        self.recording_dir = Path(self.config.output_dir) / now
    ...
    def _finalize_recording(self) -> None:
        ...
        wav_path = self.recording_dir / "audio.wav"
        ...
        self._log(f"Audio guardado en {wav_path}")
    ...
    def _run_transcription(self, wav_path: Path) -> None:
        ...
        transcript_path = self.recording_dir / "transcript.txt"
        transcript_path.write_text(result.get("text", "").strip() + "\n", encoding="utf-8")

        srt_path = self.recording_dir / "transcript.srt"
        srt_path.write_text(self._to_srt(result.get("segments", [])), encoding="utf-8")

        summary_path = self.recording_dir / "summary.md"
        summary_path.write_text(self._build_summary(result.get("text", "")), encoding="utf-8")

        self._log(f"Transcripción completada: {transcript_path}")
        self._log(f"SRT generado: {srt_path}")
        self._log(f"Resumen generado: {summary_path}")
    ...
    def _build_summary(self, transcript: str) -> str:
        # Resumen heurístico local para primera versión funcional.
